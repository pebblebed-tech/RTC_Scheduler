...
static const char *TAG = "ext_eeprom_component.component";

void ExtEepromComponent::setup() {
         if (this->begin() == false) {
             this->mark_failed();
                 ESP_LOGE(TAG, "No memory detected. Freezing.");
            }
        else
            {
                ESP_LOGD(TAG, "Memory detected!");
            }
}
...
void  ExtEepromComponent::dump_config(){
    ESP_LOGCONFIG(TAG, "Ext Eeprom component");
     LOG_I2C_DEVICE(this);
     if (this->is_failed()) {
        ESP_LOGE(TAG, "Communication with 24lc32 failed!");
     }
}
void ExtEepromComponent:: test(){
                ESP_LOGD(TAG, "Mem size in bytes: %d",this->length());
                int myValue2 = -366;
                this->put(10, myValue2); //(location, data)
                int myRead2;
                this->get(10, myRead2); //location to read, thing to put data into
                ESP_LOGD(TAG, "I read: %d",myRead2 );
       

                 float myValue3 = -7.35;
                this->put(20, myValue3); //(location, data)
                 float myRead3;
                this->get(20, myRead3); //location to read, thing to put data into
...
uint32_t ExtEepromComponent::length()
{
  return settings.memorySize_bytes;
}

//Returns true if device is detected
bool ExtEepromComponent::isConnected(uint8_t i2cAddress)
{
  if (i2cAddress == 255)
...
bool ExtEepromComponent::isBusy(uint8_t i2cAddress)
{
  if (i2cAddress == 255)
    i2cAddress = settings.deviceAddress; //We can't set the default to settings.deviceAddress so we use 255 instead
...
//void ExtEepromComponent::settings(struct_memorySettings newSettings)
//{
//  settings.deviceAddress = newSettings.deviceAddress;
//}
void ExtEepromComponent::setMemorySize(uint32_t memSize)
{
  settings.memorySize_bytes = memSize;
}
uint32_t ExtEepromComponent::getMemorySize()
{
  return settings.memorySize_bytes;
}
void ExtEepromComponent::setPageSize(uint16_t pageSize)
{
  settings.pageSize_bytes = pageSize;
}
uint16_t ExtEepromComponent::getPageSize()
{
  return settings.pageSize_bytes;
}
void ExtEepromComponent::setPageWriteTime(uint8_t writeTimeMS)
{
  settings.pageWriteTime_ms = writeTimeMS;
}
uint8_t ExtEepromComponent::getPageWriteTime()
{
  return settings.pageWriteTime_ms;
}

void ExtEepromComponent::enablePollForWriteComplete()
{
  settings.pollForWriteComplete = true;
}
void ExtEepromComponent::disablePollForWriteComplete()
{
  settings.pollForWriteComplete = false;
}
void ExtEepromComponent::setPollForWriteComplete(bool pollWriteComplete)
{
   settings.pollForWriteComplete = pollWriteComplete;
}
uint16_t ExtEepromComponent::getI2CBufferSize()
{
  return I2C_BUFFER_LENGTH_TX;
}
//Read a byte from a given location
uint8_t ExtEepromComponent::read(uint32_t eepromLocation)
{
  uint8_t tempByte;
...
void ExtEepromComponent::read(uint32_t eepromLocation, uint8_t *buff, uint16_t bufferSize)
{
  uint16_t received = 0;
...
void ExtEepromComponent::write(uint32_t eepromLocation, uint8_t dataToWrite)
{
...
void ExtEepromComponent::write(uint32_t eepromLocation, const uint8_t *dataToWrite, uint16_t bufferSize)
{
  //Error check
...
int ExtEepromComponent::writeStringToEEPROM(int addrOffset, const String &strToWrite)
{
  byte len = strToWrite.length();
  write(addrOffset, len);
  for (int i = 0; i < len; i++)
  {
    write(addrOffset + 1 + i, strToWrite[i]);
  }
  return addrOffset + 1 + len;
}
int ExtEepromComponent::readStringFromEEPROM(int addrOffset, String *strToRead)
{
  int newStrLen = read(addrOffset);
  char data[newStrLen + 1];
  for (int i = 0; i < newStrLen; i++)
  {
    data[i] = read(addrOffset + 1 + i);
  }
  data[newStrLen] = '\0'; 
